...
def send_tally_request(xml_request):
    print(f"[TALLY] Connecting to {TALLY_URL}", flush=True)
    data = xml_request.encode("utf-8")
    req = urllib.request.Request(TALLY_URL, data=data, headers={"Content-Type": "text/xml"})
    try:
        with urllib.request.urlopen(req, timeout=30) as resp:
            raw = resp.read().decode("utf-8", errors="replace")
            print(f"[TALLY] Got response: {len(raw)} bytes", flush=True)
            return sanitize_xml(raw)
    except Exception as e:
        print(f"[TALLY] ERROR: {type(e).__name__}: {str(e)}", flush=True)
        return None
...
def webhook():
    ...
    print(f"[WEBHOOK] Received: '{incoming_msg}' from {sender}", flush=True)
    ...
    try:
        ...
        print(f"[WEBHOOK] Exception: {type(e).__name__}: {str(e)}", flush=True)
